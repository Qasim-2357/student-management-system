...
from app.schemas.ai_analysis import AIPerformanceAnalysisResponse
from app.schemas.performance import PerformanceResponse
from app.security import get_current_user
from app.services.ai_analysis import get_student_ai_performance_analysis
...
@router.get(
    "/students/{student_id}/performance/analysis",
    response_model=AIPerformanceAnalysisResponse,
)
def get_student_ai_performance_analysis_endpoint(
    student_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    authorize_student_access(db, student_id, current_user)
    return get_student_ai_performance_analysis(db, student_id)
