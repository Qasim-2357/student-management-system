    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          {deleteMutation.isError ? <ErrorState title="Class could not be deleted" message={deleteMutation.error instanceof ApiError ? deleteMutation.error.message : 'Please try again.'} /> : null}
          {classes.isSuccess && classes.data.items.length === 0 ? <EmptyState title={params.search ? 'No matching classes' : 'No classes yet'} description={params.search ? 'Try adjusting your search.' : 'Class records will appear here when they are added.'} action={params.search ? <Button variant="outline" size="sm" onClick={() => updateUrl({ search: undefined, page: undefined })}>Clear search</Button> : <RoleGate roles={['admin']}><Link className={buttonVariants({ size: 'sm' })} href="/classes/new">Add class</Link></RoleGate>} /> : null}
