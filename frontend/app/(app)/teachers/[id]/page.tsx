...
export default function TeacherDetailPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm uppercase tracking-[0.2em] text-muted-foreground">Teacher</p><h1 className="mt-1 text-3xl font-semibold">{value.name}</h1></div><div className="flex gap-2"><Link className={buttonVariants({ variant: 'outline' })} href="/teachers">Back</Link><RoleGate roles={['admin']}><Link className={buttonVariants()} href={`/teachers/${value.id}/edit`}>Edit</Link></RoleGate></div></div><Card><CardHeader><CardTitle>Teacher details</CardTitle></CardHeader><CardContent><dl className="grid gap-4 sm:grid-cols-2"><div><dt className="text-sm text-muted-foreground">Teacher ID</dt><dd className="mt-1 font-mono font-medium">{value.teacher_code}</dd></div><div><dt className="text-sm text-muted-foreground">Email</dt><dd className="mt-1 font-medium">{value.email}</dd></div><div><dt className="text-sm text-muted-foreground">Phone</dt><dd className="mt-1 font-medium">{value.phone}</dd></div></dl></CardContent></Card></div>;
}
